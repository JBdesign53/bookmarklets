...
This bookmarklet allows Web Remote (browser tabs) to be quickly opened for multiple devices in Datto RMM. When activated, a yellow button is added to the UI labelled Web Remote: Control Screens which opens a control screen for each, selected device.

This removes the need to click the Web Remote button and Control Screen option for each device. Which let's be honest is a PITA, particularly for runup.
...
The image above outlines the steps to open multiple web remote tabs. These steps are:
  ...
  Note: Only selected devices, with a Web Remote button, will open.
